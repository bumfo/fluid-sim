...
window.LifeSim = function(canvasId, options) {
  ...
  options.initCFn = options.initCFn || [
    // 'step(1.0, mod(floor((x + 1.0) *0.5*'+SS+'.0) + floor((y + 1.0) *0.5*'+SS+'.0), 2.0))',
    // 'step(1.0, mod(floor((x + 1.0) *0.5*'+SS+'.0) + floor((y + 1.0) *0.5*'+SS+'.0), 2.0))',
    // 'step(1.0, mod(floor((x + 1.0) *0.5*'+SS+'.0) + floor((y + 1.0) *0.5*'+SS+'.0), 2.0))'
    'step(0.5, random(vec2(x, y)))',
    'step(0.5, random(vec2(x, y)))',
    'step(0.5, random(vec2(x, y)))',
  ];
  ...
  if (options.decayColor === undefined) {
    options.decayColor = false;
  }

  if (options.decayV === undefined) {
    options.decayV = false;
  }
  ...
  // We arbitrarily set our Life's density to 1 (this is rho in equations)
  var DENSITY = 1.0;
  ...
  // showing the direction of the Life flow.
  ...
  var addSplat = (function() {
    ...
    // Set a "splat" of change to a given place with a given
  // blob radius. The effect of the splat has an exponential falloff.
  var applyChange = (function() {
    var shader = new gl.Shader(standardVertexShaderSrc, '\
      ...
      void main() { \
        vec4 cur = texture2D(inputTex, textureCoord); \
        gl_FragColor = cur + change; \
      } \
    ');

    return function(inputTexture, change) {
      inputTexture.bind(0);
      shader.uniforms({
        change: change,
        inputTex: 0
  ...
  gl.onupdate = function() {
    ...
    //   // eventually converging to the grey-ish average color of the whole Life
    ...
    // if (options.decayColor) {
    //   textures.color1.drawTo(function() {
    //     applyDecay(
    //       textures.color0,
    //       [0.995, 0.995, 0.995, 0.0],
    //     );
    //   });
    //   textures.swap('color0', 'color1');
    // }
